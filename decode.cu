...
namespace nvinfer1
{
    ...
    __global__ void CalDetection(const float *input, float *output, int num_elem, int step, int anchor, int output_elem) {
        ...
        int y = idx / w;
        int x = idx % w;
        const float* cur_input = input + bn_idx * (4 + 2 + 8) * 2 * total_grid;
        ...
        for (int k = 0; k < 2; ++k) {
            ...
            for (int i = 0; i < 8; i += 2) {
                det->landmark[i] = prior[0] + lmk_reg[idx + k * total_grid * 8 + total_grid * i] * 0.1 * prior[2];
                det->landmark[i+1] = prior[1] + lmk_reg[idx + k * total_grid * 8 + total_grid * (i + 1)] * 0.1 * prior[3];
